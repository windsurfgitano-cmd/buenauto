...
export function EditListingForm({ listing, brands }: Props) {
  ...
  const [transmission, setTransmission] = useState(listing.transmission || "Manual");
  const [fuel, setFuel] = useState(listing.fuel || "Bencina");
  ...
  useEffect(() => {
    ...
    async function loadModels(nextBrand: string) {
      ...
      try {
        const res = await fetch(`/api/catalog?brand=${encodeURIComponent(nextBrand)}`);
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const data = (await res.json()) as UpdateListingResponse;

      if (!res.ok) {
        setError("error" in data ? data.error : "No se pudo guardar");
        return;
      }
      ...
      router.push(`/autos/${data.listing.id}`);
      router.refresh();
    } catch {
      setError("No se pudo guardar");
    } finally {
